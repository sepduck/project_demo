...
export class CreateUserComponent implements OnInit {
  ...
  private _processFormSubmission(): void {
    const roleNames = this.getRoleNamesFromIds(this.roles);
  ...
  private getRoleNamesFromIds(roleIds: number[]): string {
    if (!roleIds || roleIds.length === 0) return '';

    const selectedRoles = this.rolesSelect
